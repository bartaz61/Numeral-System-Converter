...
void frombase10(long long int number,unsigned int base,char convertednumber[]){
    ...
    //� symbol from value 128, means that number is negative.
...
void convertbase(char number[], size_t size, unsigned int frombase, unsigned int tobase){
    ...
    long long int decimalnumber = tobase10(number, size, frombase);
...
void frombase10f(/*long1*/ /*double*/ float number, unsigned int base, char convertednumber[]) {
    ...
    if (integer != 0 && base > 1) {
        ...
        do {
            push(&OPstack, (fractional % base) + 33);
            fractional /= base;
        } while (fractional > 0);
